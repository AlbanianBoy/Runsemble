...
// Four tabs flanking a raised central "Start Run" action. The Runs (hotspots)
// timeline lives inside the Map tab as a Map/Runs toggle, keeping the nav to a
// clean, modern five-target layout.
...
export function BottomNav() {
  const { activeTab, setActiveTab, openRunTracker } = useRunsembleStore()
...
function NavButton({
  ...
  return (
    ...
    >
      <motion.div animate={{ scale: active ? 1.12 : 1, y: active ? -1 : 0 }} transition={{ type: 'spring', stiffness: 400, damping: 25 }}>
